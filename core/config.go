...
func InitConfig(options *libs.Options) error {
	...
	if !utils.FileExists(options.ConfigFile) {
		...
		v.SetDefault("Cdn", map[string]string{
			// this is default s3 bucket for public report that generated by osmedeus
			"public_s3_bucket": utils.GetOSEnv("PUBLIC_S3_BUCKET", "PUBLIC_S3_BUCKET"),
			// this should be a private s3 bucket for private report that only own by you
			"private_s3_bucket": utils.GetOSEnv("PRIVATE_S3_BUCKET", "PRIVATE_S3_BUCKET"),
			"aws_access_key":    utils.GetOSEnv("AWS_ACCESS_KEY", "AWS_ACCESS_KEY"),
			"aws_secret_key":    utils.GetOSEnv("AWS_SECRET_KEY", "AWS_SECRET_KEY"),
		})
