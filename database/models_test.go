...
func TestDnsModels(t *testing.T) {
    ...
    DB.Where("asset_value = ?", "academy.live-test.shopee.io").First(&assetObj)
    if assetObj.ID == 0 {
        assetObj = Asset{
            AssetValue: "academy.live-test.shopee.io",
            //Dns:        []Dns{*d},
...
func TestQueryModels(t *testing.T) {
    ...
    //var assetObj Asset
    //tx:= DB.Preload("Dns.HTTP.Directory.Vulnerability.Link.Archive").Where("asset_value = ?", "academy.live-test.shopee.io").First(&assetObj)
...
func TestSingleQueryModels(t *testing.T) {
    ...
    //tx:= DB.Preload("Dns").Preload("Http").Where("asset_value = ?", "academy.live-test.shopee.io").First(&assetObj)
    DB.Table("Assets").Preload("Link").Preload("Http").Where("asset_value = ?", "dean.shopee.io").First(&assetObj)
    spew.Dump(assetObj)
}
